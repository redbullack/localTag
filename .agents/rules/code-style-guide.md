...
## 4. 컴포넌트 및 유틸리티 재사용 (Reuse & DRY)
* **기존 컴포넌트 우선 사용:** 새로운 UI를 작성하기 전에 `components/` 폴더에 이미 동일 역할의 컴포넌트가 존재하는지 반드시 확인하세요. 존재한다면 해당 컴포넌트를 import하여 사용하고, HTML 마크업을 직접 복붙하지 마세요.
* **유틸리티 함수 중복 금지:** `utils/` 폴더에 이미 정의된 함수와 동일하거나 유사한 로직을 개별 컴포넌트 내부에 다시 정의하지 마세요. 반드시 기존 유틸을 import하여 사용하세요.
* **반복 로직은 커스텀 훅으로 추출:** 2곳 이상에서 동일한 `useEffect` 패턴이나 상태 관리 로직이 반복되면, `utils/` 폴더에 커스텀 훅(`use-*.ts`)으로 추출하세요. (예: 외부 클릭 감지 → `useClickOutside`)
* **공유 상수 관리:** 여러 파일에서 사용되는 매직 넘버, 색상 코드, 문자열 등은 상수로 정의하고 한 곳에서 export하여 사용하세요. 개별 파일에 같은 값을 하드코딩하지 마세요.